...
class DependencyFixer:
    ...
    def create_files_tar(self, source_dir: Optional[Path] = None) -> bool:
        ...
        # 创建 .linyaps-genius 标记文件
        genius_file = source_dir / ".linyaps-genius"
        try:
            timestamp = datetime.now().isoformat()
            genius_file.write_text(f"Generated by linyaps dependency fixer\nTimestamp: {timestamp}\n", encoding="utf-8")
            print(f"✓ Created .linyaps-genius marker file: {genius_file}")
        except Exception as e:
            print(f"✗ Failed to create .linyaps-genius marker file: {e}")
            # 标记文件创建失败不影响主流程，继续执行
